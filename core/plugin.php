...
use MagicWand\Core\Managers\Modules;
...
class Plugin {
    ...
    public function __construct() {
        ...
        require_once(MAGIC_WAND_PATH . 'core' . DIRECTORY_SEPARATOR . 'helper.php');
    ...
    public static function instance() {
        if (is_null(self::$instance)) {
             ...
             * @since 1.0.1
             */
            do_action('magic-wand/instance', self::$instance);
        }
    ...
    public function autoload($class) {
        ...
        if (!class_exists($class)) {
            $filename = \MagicWand\Core\Helper::class_to_path($class);
            if (is_readable($filename)) {
                include_once( $filename );
                //var_dump($filename);
            } else {
                // fallback
                $plugin_path = \MagicWand\Core\Helper::get_plugin_path($class);
    ...
    public function setup_hooks() {
        ...
        do_action('magic-wand/init');
    }

    public function get_name() {
        list($class, $none) = explode("\\", get_class($this), 2);
        $slug = \MagicWand\Core\Helper::camel_to_slug($class);
        return $slug;
    }

    public function maybe_vendor_autoload($TextDomain = '') {
        if ($this->has_vendors($TextDomain)) {
            $file = MAGIC_WAND_PATH . DIRECTORY_SEPARATOR . 'vendor' . DIRECTORY_SEPARATOR . 'autoload.php';
            if (file_exists($file)) {
    ...
    public function has_vendors($TextDomain = '') {
        $composer = MAGIC_WAND_PATH . DIRECTORY_SEPARATOR . 'composer.json';
